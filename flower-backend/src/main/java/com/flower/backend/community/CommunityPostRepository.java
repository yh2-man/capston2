...
public interface CommunityPostRepository extends JpaRepository<CommunityPost, Long> {
    ...
    @Query("SELECT p FROM CommunityPost p WHERE p.postType = 'GENERAL' ORDER BY p.createdAt DESC")
    List<CommunityPost> findFeed(Pageable pageable);

    @Query("SELECT p FROM CommunityPost p WHERE p.postType = 'GENERAL' AND p.id < :cursor ORDER BY p.createdAt DESC")
